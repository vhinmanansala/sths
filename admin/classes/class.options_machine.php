...
class Options_Machine {
			...
			switch ( $value['type'] ) {
				...
				case 'courses':
					...
					if ($count < 2) {
						$oldorder = 1;
						$order = 1;
						$output .= Options_Machine::optionsframework_course_function($value['id'],$value['std'],$oldorder,$order);
					} else {
						$i = 0;
						foreach ($slides as $slide) {
							$oldorder = $slide['order'];
					...
					$output .= '</ul>';
					$output .= '<a href="#" class="button course_add_button">Add New Course</a></div>';
	...
	public static function optionsframework_course_function($id,$std,$oldorder,$order){
		...
		if (!empty($val['title'])) {
			...
			$slider .= '<li><div class="slide_header"><strong>Course '.$order.'</strong>';
		}
		...
		$slider .= '<a class="course_delete_button" href="#">Delete</a>';
	    $slider .= '<div class="clear"></div>' . "\n";
